...
/**
 * Apply version-specific Alkaka patches to the openclaw source tree.
 *
...
// This removes stale patches left by a different Alkaka branch that may have
...
for (const patchFile of patchFiles) {
  ...
  if (needsNormalize) {
    patchPath = path.join(os.tmpdir(), `alkaka-patch-${patchFile}`);
