...
resolve_node_runtime() {
  ...
  if [ -n "${ALKAKA_ELECTRON_PATH:-}" ] && [ -x "${ALKAKA_ELECTRON_PATH}" ]; then
    NODE_CMD="$ALKAKA_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from Alkaka."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from Alkaka so scripts can use Electron runtime."
  exit 1
fi
