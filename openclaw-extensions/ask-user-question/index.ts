 ...
 * and waits for the user's response via an HTTP callback to Alkaka.
 *
 * This enables delete-confirmation modals on the Alkaka desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for Alkaka desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool((ctx) => {
      // Only enable for Alkaka desktop sessions (sessionKey starts with 'agent:main:alkaka:').
      ...
      const isLocalDesktop = sessionKey.startsWith('agent:main:alkaka:');
      if (!isLocalDesktop) {
        return null;
      }
