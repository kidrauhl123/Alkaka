...
- 最新功能范围：**Alkaka Chat 真实 Cowork/OpenClaw 消息流、真实空白首页与桌宠状态接入**。
- 已完成到：`AlkakaProjectChatHome` 新增真实消息时间线 mapper，把 `CoworkMessage[]` 映射为聊天流；支持 `user / assistant / system / tool_use / tool_result`，工具调用与工具结果作为真实消息卡显示。没有真实 sessions 时显示真实空白首页和新建入口，不预置假的伙伴/智能体/项目组/聊天内容；打开真实 current session 但消息为空时显示真实空态，不再回退到 AI 日报示例。
- `CoworkView` 不再在打开真实 session 后切回旧详情页，而是保持 Alkaka Chat 三栏 shell：左栏导航收敛为 `Skill` 与 `设置`，最近对话只显示真实会话；主聊天区接 `currentSession.messages`，composer 对首页调用 `handleStartSession()` 新建真实 Cowork/OpenClaw session、对已打开 session 调用 `handleContinueSession()`；打开 running 会话后保留真实停止、置顶、重命名、删除入口，避免旧详情页关键 runtime 操作回归。
- 右侧工作台核心状态接真实 `sessions`、`currentSessionId` 和 `OpenClawEngineStatus`：活跃/总数/完成/异常 session、当前状态、OpenClaw phase、最新会话标题都由真实状态推导；无真实 session 时明确显示空态，不展示默认伙伴头像或假成员。
...
- 验证命令：
  ...
  - `npm run build`：通过。
  - `git diff --check`：通过。
- 下一优先级：把右侧工作台进一步接真实 OpenClaw telemetry、权限请求、产物/文件与执行日志；如没有真实数据，继续保持空态而不是填充示例。
...
- 已完成到：在参考图风格 `AlkakaProjectChatHome` 基础上，左侧“最近对话”继续优先消费真实 Redux Cowork session summaries；本轮新增 `buildProjectGroupPreview()`，把中间主聊天区从固定 AI 日报项目组进一步推进为真实 session 预览：有真实会话时展示真实标题、`OpenClaw 会话预览`、创建时间、运行状态、置顶对话和最近更新时间；没有真实会话时才回退到 AI 日报示例内容。
...
- 桌宠形象、真实像素命中、透明区穿透、单击/双击行为和窗口位置逻辑未改动。
- 验证命令：
  - `npx vitest run src/renderer/components/chat/AlkakaProjectChatHome.test.ts`：1 file / 13 tests passed。
  - `npx vitest run src/renderer/components/chat/AlkakaProjectChatHome.test.ts src/renderer/components/pet/petTaskJump.test.ts src/renderer/components/pet/petState.test.ts src/main/petStatus.test.ts src/renderer/components/pet/petQuickTask.test.ts`：5 files / 28 tests passed。
  - `npm run compile:electron -- --pretty false`：通过。
  - `npm run build`：通过。
  - `git diff --check`：通过。
- 下一优先级：继续把当前只是 summary 级别的中间预览推进为真实 Cowork messages / tool cards 渲染，并补 `CoworkView` integration 测试。
