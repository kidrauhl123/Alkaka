...
export async function downloadUpdate(
  ...
  const ts = Date.now();
  const downloadPath = path.join(updateDir, `alkaka-update-${source}-${ts}${ext}.download`);
  const finalPath = path.join(updateDir, `alkaka-update-${source}-${ts}${ext}`);
...
async function installWindowsNsis(exePath: string): Promise<void> {
  ...
  // the NSIS customInit macro runs `taskkill /IM "Alkaka.exe" /F /T`
  ...
  const logPath = path.join(tempDir, `alkaka-update-${ts}.log`);
  const scriptPath = path.join(tempDir, `alkaka-update-${ts}.ps1`);
  const vbsPath = path.join(tempDir, `alkaka-update-${ts}.vbs`);
