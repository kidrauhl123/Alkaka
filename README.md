...
- 左栏包含品牌、渐变“新建对话”、搜索、`Skill` 与 `设置` 两个轻量导航入口和最近真实会话；中间无真实 session 时显示空白新建提示，有真实 session 时显示会话预览、真实消息卡片、工具调用/结果卡片和 composer；右栏是真实 Cowork/OpenClaw 链路状态、最近真实会话和真实能力入口。
