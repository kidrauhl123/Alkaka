...
| # | 任务 | 状态 | 下一步 |
|---|------|------|--------|
| 2.1 | 把 `openclawRuntimeAdapter` 的接口提炼成 `AgentEngineProtocol` / Router 多 runtime map | ✅ checkpoint | 后续命名可继续收敛 |
| 2.2 | 接入 Hermes 作为第二引擎（PoC：先跑通基本对话 + tool use） | ⏭️ 下一步 | 先做 mock adapter，不急接真实 runtime |
| 2.3 | 设置里加引擎切换 UI + 引擎能力差异说明 | 计划 | 等 Hermes adapter 可用后再开放 |
| 2.4 | 双引擎跑通 IM 网关（验证两个 engine 都能通过同一套 IM gateway 对外） | 计划 | 在 2.2/2.3 后做 |
