...
test('parseManagedSessionKey handles raw local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('alkaka:abc-123'), {
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('agent:main:alkaka:abc-123'), {
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  assert.equal(
    ...
    `agent:${DEFAULT_MANAGED_AGENT_ID}:alkaka:abc-123`,
  );
  assert.equal(
    buildManagedSessionKey('abc-123', 'secondary'),
    'agent:secondary:alkaka:abc-123',
  );
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  assert.equal(parseChannelSessionKey('alkaka:abc-123'), null);
  assert.equal(parseChannelSessionKey('agent:main:alkaka:abc-123'), null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  assert.equal(isManagedSessionKey('agent:main:alkaka:abc-123'), true);
  assert.equal(sync.isChannelSessionKey('agent:main:alkaka:abc-123'), false);
  assert.equal(sync.resolveOrCreateSession('agent:main:alkaka:abc-123'), null);
  assert.equal(sync.resolveOrCreateMainAgentSession('agent:main:alkaka:abc-123'), null);
});
