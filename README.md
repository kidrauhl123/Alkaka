...
git clone https://github.com/kidrauhl123/Alkaka.git
cd Alkaka
...
Alkaka can use [OpenClaw](https://github.com/openclaw/openclaw) as its agent engine.
...
The bundled runtime is interpreter-focused and does not preinstall Alkaka skill Python packages; those can be installed at runtime on demand.
...
Alkaka uses Electron's strict process isolation. All cross-process communication goes through IPC.
...
Cowork is the core feature of Alkaka — an AI working session system powered by OpenClaw as the primary agent engine. Designed for productivity scenarios, it can autonomously complete complex tasks like data analysis, document generation, and information retrieval.
...
Alkaka ships with 29 built-in skills covering productivity, creative, investment research, and automation scenarios, configured via `SKILLs/skills.config.json`:
...
Alkaka supports scheduled tasks that let the Agent automatically execute recurring work on a set schedule.
...
Alkaka can bridge the Agent to multiple IM platforms. Send a message from your phone via IM to remotely trigger the desktop Agent — command your personal assistant anytime, anywhere.
...
Alkaka's memory system is built on OpenClaw and persists information as files in the working directory, so the Agent remembers your preferences and context across sessions.
...
Alkaka enforces security at multiple layers:
...
Alkaka pins its OpenClaw dependency to a specific release version, declared in `package.json`:
