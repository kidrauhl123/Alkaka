...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.ALKAKA_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'Alkaka Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'Alkaka Skill Downloader',
...
export class SkillManager {
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            ...
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'alkaka-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'alkaka-skill-npm-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'alkaka-skill-clawhub-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'alkaka-skill-'));
        cleanupPath = tempRoot;
  ...
  private async performUpgradeDownload(skillId: string, downloadUrl: string, existingSkillDir: string): Promise<{
    ...
    try {
      ...
      const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'alkaka-skill-upgrade-'));
      cleanupPath = tempRoot;
