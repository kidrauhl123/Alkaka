...
| 2.2 | OpenClaw runtime 稳定性收敛：启动、配置同步、错误恢复、日志定位 | ⏭️ 下一步 | 先跑真实产品链路并修阻塞问题 |
...
2026-04-28　路线调整：暂缓 Hermes，先稳定 OpenClaw
  · 基于“本项目已深度使用 OpenClaw”的判断，Phase 2.2 不再做 Hermes adapter
  · 下一步改为 OpenClaw runtime 真实产品链路验证：启动、配置同步、错误恢复、日志定位、IM/Cowork 端到端
  · 保留 Router/Protocol 边界，避免回到硬耦合；但不继续为了第二引擎扩大抽象
```
